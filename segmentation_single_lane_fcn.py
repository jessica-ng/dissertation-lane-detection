...
import numpy as np
import utils.singleLaneLabels as labels
...
def import_test_data():
    ...
    dataset = dataset.data_batch(batch_size=10,
                                 augment=False,
                                 shuffle=False)
    ...
    for image, mask in dataset:
        ...
        test_labels.append(labels.getLabels(mask))

    return [test_images, test_mask, test_labels]
...
def load_model():
    ...
    checkpoint_path = "training_single_lanes_fcn_1/cp.ckpt"
    ...
    model.load_weights('training_single_lanes_fcn_1/weights.h5')
...
def train(model):
    ...
    checkpoint_path = "training_single_lanes_fcn_2/cp.ckpt"
    checkpoint_dir = os.path.dirname(checkpoint_path)
    ...
    model_history = model.fit(train_dataset[0][0], train_dataset[2][0], epochs=EPOCHS,
                              batch_size=BATCH_SIZE,
                              steps_per_epoch=100,
                              validation_steps=100,
                              validation_data=(test_dataset[0][0], test_dataset[2][0]))
    ...
    model.save_weights('training_single_lanes_fcn_2/weights.h5')
    model.save('training_single_lanes_fcn_2/fcn.h5')
