...
def create_model():
    ...
    model = applications.resnet50.ResNet50(weights= None, include_top=False, input_shape= (128,128,3))
    x = model.output
    ...
    x = Conv2DTranspose(3, (3,3), strides=(2, 2), activation='relu', padding='same')(x)
    x = Conv2DTranspose(3, (3,3),strides=(2, 2), activation='relu', padding='same')(x)
    x = Conv2DTranspose(3, (3,3),strides=(2, 2), activation='relu', padding='same')(x)
    x = Conv2DTranspose(3, (3, 3), strides=(2, 2), activation='relu', padding='same')(x)
    predictions = Conv2DTranspose(3, (3,3),strides=(2, 2), activation='relu', padding='same')(x)
    #predictions = Dropout(0.5)(x)
    # predictions = Dense(2, activation="softmax")(x)
    #x = Conv2DTranspose(3, 3, strides=2, padding='same', activation='softmax')(x)
    # x = Conv2DTranspose(3, 3, strides=2, padding='same', activation='softmax')(x)
    # x = Conv2DTranspose(3, 3, strides=2, padding='same', activation='softmax')(x)
    # x = Conv2DTranspose(3, 3, strides=2, padding='same', activation='softmax')(x)
    #predictions = Conv2DTranspose(3, 3, strides=2, padding='same', activation='softmax')(x)
...
def load_model():
    ...
    checkpoint_path = "training_lanes_resnet_4/cp.ckpt"
    ...
    model.load_weights('training_lanes_resnet_4/weights.h5')
...
def show_predictions( model, dataset=None, num=1):
    ...
    if dataset:
        ...
        display([sample_image, sample_mask,
                 create_mask(model.predict(sample_image, steps=1))])
...
def train(model):
    ...
    model.save_weights('training_lanes_resnet_4/weights.h5')
    model.save('training_lanes_resnet_4/resnet.h5')
